...
if (isset($_SESSION['email'])) {
    ...
    $sql = "SELECT id, first_name, profile_picture FROM users WHERE email = ?";
...
// Fetch bookings
$sql = "SELECT bookings.id as booking_id, consultants.name as consultant_name, bookings.date, bookings.time 
        FROM bookings 
        JOIN consultants ON bookings.consultant_id = consultants.id 
                ...
                <?php if ($bookings && $bookings->num_rows > 0): ?>
                    <?php while($row = $bookings->fetch_assoc()): ?>
                        <div class="booking-item">
                            ...
                            <div class="actions">
                                <button class="button" onclick="showCancelModal(<?php echo $row['booking_id']; ?>)">Cancel</button>
                                <button class="button" onclick="showRescheduleModal(<?php echo $row['booking_id']; ?>)">Reschedule</button>
                                <a href="chat_form.php?consultation_id=<?php echo $row['booking_id']; ?>" class="button">Chat</a>
                            </div>
                        </div>
                    <?php endwhile; ?>
                <?php else: ?>
